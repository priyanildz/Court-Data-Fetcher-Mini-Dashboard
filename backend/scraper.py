...
async def fetch_case_details_async(case_type, case_number, filing_year):
    ...
    try:
        async with async_playwright() as p:
            ...
            await page.wait_for_selector(case_type_selector, state='visible', timeout=10000)
            ...
            print(f"DEBUG: Attempting to select Case Type with value: '{case_type}' (keyboard interaction)")
            ...
            try:
                ...
                selected_value_after_attempt = await page.evaluate(f"document.querySelector('{case_type_selector}').value")
                if selected_value_after_attempt == case_type.strip():
                    print(f"SUCCESS: Case Type confirmed as '{selected_value_after_attempt}' via keyboard interaction.")
                else:
            ...
            await page.wait_for_selector(case_number_selector, state='visible', timeout=10000)
            
            await page.focus(case_number_selector)
            await asyncio.sleep(0.1)
            await page.type(case_number_selector, case_number, delay=50)
            
            filled_case_number = await page.evaluate(f"document.querySelector('{case_number_selector}').value")
            ...
            await page.wait_for_selector(filing_year_selector, state='visible', timeout=10000)

            print(f"DEBUG: Attempting to select Filing Year with value: '{str(filing_year)}'")
            try:
                ...
                selected_year_after_attempt = await page.evaluate(f"document.querySelector('{filing_year_selector}').value")
                if selected_year_after_attempt == str(filing_year):
                    print(f"SUCCESS: Filing Year confirmed as '{selected_year_after_attempt}'")
                else:
            ...
            try:
                ...
                try:
                    ...
                    no_results_selector = 'div.tab-pane.container.active p:has-text("No records found")'
                    try:
                ...
                await page.wait_for_load_state('networkidle')
                print(f"Navigated to results or results loaded on page. Current URL: {page.url}")
            ...
            try:
                result_row = page.locator('div.tab-pane.container.active tr:has(a[href*="case-type-status-details"])').first
                if await result_row.count() > 0:
                    ...
                    parties_text = parties_html.replace('<br>', ' ').replace('&nbsp;', ' ').strip()
                    parties_names = [p.strip() for p in re.split(r'\s+V\.S\.\s+|\s+V\.S\.\s+', parties_text, flags=re.IGNORECASE) if p.strip()]
                    case_data['parties'] = " vs. ".join(parties_names) if parties_names else 'N/A'
                    
                    # Dates are in the 4th column (0-indexed 3)
                    dates_td = result_row.locator('td').nth(3)
                    dates_text = await dates_td.inner_text()
                    
                    next_date_match = re.search(r'NEXT DATE: (.*?)(?:<br>|\n|$)', dates_text)
                    last_date_match = re.search(r'Last Date: (\d{2}/\d{2}/\d{4})', dates_text)
                    ...
                    order_link_locator = result_row.locator('a[href*="case-type-status-details"]').first
                    if await order_link_locator.count() > 0:
                        ...
                        if href:
                            ...
                            case_data['orders'] = [{
                                ...
                                'description': order_desc.strip() if order_desc.strip() else 'Case Details',
                                'link': order_link,
...
if __name__ == '__main__':
    ...
    details, raw_response, err = fetch_case_details(test_case_type, test_case_number, test_filing_year)
    ...
    if err:
        ...
        if details['orders']:
            for order in details['orders']:
                print(f"  - {order['date']}: {order['description']} ({order['link']}) ({order['filename']})")
        else:
